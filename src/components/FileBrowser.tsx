...
import { TagManager, getAllTags, getDocumentTags, type DocumentTag } from './TagManager';
...
export function FileBrowser({
  ...
  }, [documents]); // Reload tags when documents change
  ...
  const filteredDocuments = useMemo(() => {
    ...
    if (selectedTagIds.length > 0) {
      result = result.filter((doc) => {
        const docTagIds = getDocumentTags(doc.id);
  ...
  const handleDeleteDocument = async (docId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!confirm('確定要將這個文件移到垃圾桶嗎？')) return;
    ...
    >
      ...
      <div className="p-4 border-b border-zinc-200 dark:border-zinc-800">
        ...
        {availableTags.length > 0 && (
          <div className="relative mt-2" ref={tagFilterRef}>
            <button
            ...
            >
              ...
              {selectedTagIds.length > 0 ? (
                <span>{selectedTagIds.length} 個標籤</span>
              ) : (
                <span>標籤過濾</span>
              )}
            </button>
            ...
            {showTagFilter && (
              <div className="absolute top-full left-0 mt-1 w-48 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-lg z-50 overflow-hidden">
                <div className="p-2 border-b border-zinc-200 dark:border-zinc-700 flex items-center justify-between">
                  <span className="text-xs font-medium text-zinc-600 dark:text-zinc-400">選擇標籤</span>
                  {selectedTagIds.length > 0 && (
                    <button
